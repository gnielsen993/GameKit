...
last_updated: 2026-06-27T14:37:27.264Z
...
- 2026-05-01: **iCloud + Background Modes capabilities registered** (commit `bef885d`). `gamekit.entitlements` + `Info.plist` + pbxproj all updated via Xcode UI per CLAUDE §8.8 (no hand-patching). Fixes the SIWA-tap crash flagged by real-device test ("CKContainer.m:748: ... entitlement missing"). Plan 06-03 Task 3's UAT-signed-off claim was wrong — capabilities had never actually been added; this commit closes that gap. Container literal `iCloud.com.lauterstar.gamekit` now matches across PROJECT.md + GameKitApp.swift + ModelContainerSmokeTests + entitlements (4-place lock preserved).
- 2026-05-01: **PF-03 / PF-04 / SC1-B closed — CloudKit Production schema deployed.** Plan 06-03's lldb-driven `_runtimeDeployCloudKitSchema()` flow doesn't work in practice (paused frame lands in libsystem_kernel where Swift module symbols don't resolve), so replaced with a DEBUG-only auto-deploy block in `GameKitApp.init()` (commit `36d65c6`) — UserDefaults-gated, runs once per device, prints success/failure to console, stripped from Release builds. Dev schema materialized CD_GameRecord + CD_BestTime + CD_BestScore (Merge BestScore added to the original P6 two-type spec). Dashboard "Deploy Schema to Production" button promoted Dev → Prod. Production env verified to have all three CD_* types. **Pre-flight blocker for TestFlight upload removed.** Pitfall 11 row "CloudKit container not provisioned for Production" → mitigated (SC3-C..F still pending the actual TestFlight build).
- 2026-05-01: **Game-screen UX hardening** (commit `08d4bee`). Merge dropped from `UpcomingGamesView.upcomingGames` (graduated to playable; HomeView caption "8 games coming" → "7"). MinesweeperGameView + MergeGameView now hide the system back chevron + edge-swipe-to-go-back gesture (`.navigationBarBackButtonHidden(true)`) and ship a custom back ToolbarItem at `.topBarLeading`. Merge's swipe-driven tile interactions were getting hijacked by the system gesture near the left edge; same treatment applied to Minesweeper for consistency (cell long-press + pinch-to-zoom near left edge had the same hijack risk). A11y label "Back to The Drawer" preserved.
- 2026-05-01: **Restart-alert UX fixed** (commit `89c529f`). Hardcoded "GameKit" in the iCloud restart prompt → interpolates `AppInfo.displayName`. Misleading "Restart to enable iCloud sync" + "Quit GameKit" labels → "Quit and reopen to finish iCloud setup" + "Later" / "OK". The dismiss-only invariant from Plan 06-06 D-05 LOCK (no programmatic termination per Apple Review) is preserved structurally — both buttons still have empty bodies.
...
- Plan 11-03: split MinesweeperGameView into a sibling-file extension (MinesweeperGameView+VideoMode.swift) to stay under the CLAUDE.md §8.5 ≤500-line cap; dropped private modifier from properties consumed by the extension (Rule 3 deviation — Swift file-scoped private is incompatible with separate-file extensions).
- Plan 11-03: Large-zone branch ships as a TODO-11-04 stub that hides the toolbar; Plan 11-04 fills in the VideoCompactControlRow composition per D-05 slot order.
- Plan 11-03: SlotAnchor → ToolbarItemPlacement mapping is defensively exhaustive (6-case switch) — bottom anchors map to .bottomBar (forward-compat for future Small-zone refinements); inCompactRow/hidden map defensively to .topBarLeading (unreachable on Small zones today).
- 11-04: Stacked-chip-in-slot-2 pattern lands in Mines Large-zone branch (D-06). VideoCompactControlRow component preserved verbatim (D-07 5-slot contract intact); slot 2's primaryInfo closure hosts a VStack of MinesRemainingChip + TimerChip. .reducedTime drops only the TimerChip half.
...
Last session: 2026-06-27T02:50:47.188Z
