...
struct MinesweeperModePill: View {
    ...
    var body: some View {
        ...
        .accessibilityValue(Text(mode == .reveal ? String(localized: "Reveal") : String(localized: "Flag")))
    ...
    private func segment(_ target: MinesweeperInteractionMode,
        ...
        .background {
            if isActive {
                Capsule()
                    .fill(target == .flag ? theme.colors.danger : theme.colors.accentPrimary)
                    .matchedGeometryEffect(id: "activeSegment", in: pillNamespace)
            }
        }
    }

    private var toggledMode: MinesweeperInteractionMode {
