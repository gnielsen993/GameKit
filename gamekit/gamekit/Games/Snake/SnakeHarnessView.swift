//
//  SnakeHarnessView.swift
//  gamekit
//
//  THROWAWAY — deleted at Phase 17 start, replaced by SnakeGameView().
//
...
//      avoids unintended start from .idle (guarded in SnakeHarnessVM)
...
// MARK: - SnakeHarnessView

struct SnakeHarnessView: View {

    @EnvironmentObject private var themeManager: ThemeManager
    @Environment(\.colorScheme) private var colorScheme
    @Environment(\.scenePhase) private var scenePhase
    @Environment(\.dismiss) private var dismiss
    @State private var vm = SnakeHarnessVM()
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: theme.spacing.l) {
                // Oscillating dot: vertical sine wave derived from tickCount.
                // Vertical oscillation distinguishes Snake visually from Stack's
                // horizontal movement, while confirming the loop is running.
                let angle = Double(vm.tickCount) * (2 * .pi / 120.0)
                let yOffset = CGFloat(sin(angle) * 100)
                Circle()
